...
def main():
    ...
    Initialize model
    '''
    model = BFMMorph.BFMMorph(config=BFMMorph.get_3DBFMMorph_config())
    model.cuda()
    ...
    train_set = datasets.IXIBrainDataset(glob.glob(train_dir + '*.pkl'), atlas, transforms=train_composed)
    val_set = datasets.IXIBrainInferDataset(glob.glob(val_dir + '*.pkl'), atlas, transforms=val_composed)
